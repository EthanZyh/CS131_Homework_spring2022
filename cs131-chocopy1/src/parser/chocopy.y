...
expr_stmt: TOKEN_PASS {$$=new parser::PassStmt(get_loc(@$));}
    ...
    | assign_stmt expr {$1->location=get_loc(@$);$1->value=$2;$$=$1;}
    ;
assign_stmt: assign_expr TOKEN_equal {
        $$=new parser::AssignStmt(get_loc(@$),new vector<parser::Expr*>,nullptr);
        $$->targets->push_back($1);
    }
    | assign_stmt assign_expr TOKEN_equal {$$=$1;$$->targets->push_back($2);$$->location=get_loc(@$);}
    ;
assign_expr: identifier {$$=$1;}
    | member_expr {$$=$1;}
    | index_expr {$$=$1;}
    ;
