...
int main(int argc, char *argv[]) {
    ...
    if (!error->empty()) {
        ...
        if (run) {
            ...
            auto command_string = "clang -mno-relax -no-integrated-as -O0 -w --target=riscv32-unknown-elf "s +
                                  target_path + ".ll -o " + target_path + ".s -S " +
        ...
        if (assem) {
#ifdef RV64
            auto command_string = "clang -mno-relax --target=riscv64 -O0 -w -S --target=riscv64-unknown-linux-gnu "s +
                                  target_path + ".ll -o " + target_path + ".s -L. -lchocopy_stlib";
#else
            auto command_string = "clang -mno-relax --target=riscv32 -O0 -w -S --target=riscv32-unknown-linux-gnu "s +
