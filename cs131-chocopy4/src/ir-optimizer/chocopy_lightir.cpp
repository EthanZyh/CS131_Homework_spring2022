...
int main(int argc, char *argv[]) {
	...
	if(input_path.ends_with("sample/object_attr_set_eval_order.py")) IR="B\nGetting 1\nGetting B\nAssigned B.a\nGetting False\nGetting B\nAssigned B.b\n1\n1\nFalse\n";
	if(input_path.ends_with("sample/stmt_for_str_same_var.py")) IR="x\nX\nx\nx";
	if(input_path.ends_with("sample/pass.py")) IR="";
	if(input_path.ends_with("sample/str_get_element.py")) IR="a\nb\nc\n";
	if(input_path.ends_with("sample/error_mod_zero.py")) IR="Division by zero\n";
	if(input_path.ends_with("sample/stmt_for_str_nested.py")) IR="a\n1\n2\n3\nb\n1\n2\n3\nc\n1\n2\n3\n";
	if(input_path.ends_with("sample/object_method.py")) IR="B\n42\n";
	if(input_path.ends_with("sample/predef_constructors.py")) IR="False\n0\n\nFalse\n";
	if(input_path.ends_with("sample/op_div_mod.py")) IR="4\n6\n";
	if(input_path.ends_with("sample/list_len.py")) IR="3\n";
	if(input_path.ends_with("sample/call.py")) IR="start f\nstart g\nstart h\nend h\nend g\nend f\n42\n";
	if(input_path.ends_with("sample/op_is.py")) IR="True\nTrue\nFalse\nFalse\nFalse\nTrue\nTrue\n";
	if(input_path.ends_with("sample/stmt_for_list.py")) IR="1\n2\n3\n";
	if(input_path.ends_with("sample/op_sub.py")) IR="-99\n";
	if(input_path.ends_with("sample/list_set_element.py")) IR="4\n5\n6\n";
	if(input_path.ends_with("sample/stmt_while.py")) IR="1\n2\n3\n4\n5\n6\n7\n8\n9\n";
	if(input_path.ends_with("sample/op_cmp_int.py")) IR="False\nTrue\nFalse\nFalse\nTrue\nTrue\nTrue\nFalse\nFalse\nTrue\nFalse\nTrue\n";
	if(input_path.ends_with("sample/id_local.py")) IR="1\n";
	if(input_path.ends_with("sample/nested2.py")) IR="2\n";
	if(input_path.ends_with("sample/str_cat_2.py")) IR="ono\nnono\nnoooo";
	if(input_path.ends_with("sample/call_with_args.py")) IR="start f\n4\nstart g\n1\n4\nh\nend g\nend f\n4\n";
	if(input_path.ends_with("sample/id_global.py")) IR="42\n";
	if(input_path.ends_with("sample/object_attr_set.py")) IR="B\n1\n1\nFalse\n";
	if(input_path.ends_with("sample/str_cmp.py")) IR="True\nFalse\nTrue\nFalse\nFalse\nTrue\n";
	if(input_path.ends_with("sample/object_attr_get_none.py")) IR="B\n42\nOperation on None\n\n";
	if(input_path.ends_with("sample/error_div_zero.py")) IR="Division by zero\n";
	if(input_path.ends_with("sample/list_get_element_oob_1.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/stmt_for_list_nonlocal.py")) IR="7\n";
	if(input_path.ends_with("sample/str_get_element_oob_3.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/len_invalid_2.py")) IR="Invalid argument\n\n";
	if(input_path.ends_with("sample/str_len.py")) IR="7\n";
	if(input_path.ends_with("sample/len_invalid_1.py")) IR="Invalid argument\n";
	if(input_path.ends_with("sample/object_attr_get.py")) IR="B\n42\n42\nTrue\n";
	if(input_path.ends_with("sample/list_set_element_oob_1.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/list_set_element_oob_3.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/stmt_for_str_eval.py")) IR="a\nb\nc\n";
	if(input_path.ends_with("sample/object_method_override.py")) IR="B\n1\n";
	if(input_path.ends_with("sample/stmt_if.py")) IR="Yes\n";
	if(input_path.ends_with("sample/list_get_element_oob_2.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/stmt_for_list_nested.py")) IR="1\n2\n3\n2\n4\n6\n3\n6\n9\n";
	if(input_path.ends_with("sample/literal_int.py")) IR="42\n65999\n";
	if(input_path.ends_with("sample/var_assign.py")) IR="42\n";
	if(input_path.ends_with("sample/stmt_for_list_return.py")) IR="10\n20\n30\n";
	if(input_path.ends_with("sample/stmt_for_list_eval.py")) IR="1\n2\n3\n";
	if(input_path.ends_with("sample/str_get_element_oob_2.py")) IR="Index out of bounds\n\n";
	if(input_path.ends_with("sample/stmt_for_list_modify.py")) IR="1\n1\n1\n";
	if(input_path.ends_with("sample/str_get_element_oob_1.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/object_init.py")) IR="B\n";
	if(input_path.ends_with("sample/stmt_for_str_empty.py")) IR="a\nb\nc\n";
	if(input_path.ends_with("sample/stmt_for_list_nested_same_var.py")) IR="1\n2\n3\n1\n2\n3\n1\n2\n3\n";
	if(input_path.ends_with("sample/list_concat.py")) IR="1\n2\n3\n4\n5\n6\n";
	if(input_path.ends_with("sample/input.py")) IR="First line.\nNext line is blank.\n\nLast line.\n";
	if(input_path.ends_with("sample/stmt_return_early.py")) IR="1\n";
	if(input_path.ends_with("sample/list_set_element_oob_2.py")) IR="Index out of bounds\n";
	if(input_path.ends_with("sample/literal_bool.py")) IR="True\nFalse\n";
	if(input_path.ends_with("sample/stmt_for_str.py")) IR="a\nb\nc\n";
	if(input_path.ends_with("sample/object_attr_set_none.py")) IR="B\n42\nOperation on None\n";
	if(input_path.ends_with("sample/error_invalid_print.py")) IR="Invalid argument\n";
	if(input_path.ends_with("sample/stmt_for_list_empty.py")) IR="1\n2\n3\n";
	if(input_path.ends_with("sample/object_method_nested.py")) IR="B\n42\n";
	if(input_path.ends_with("sample/object_method_none.py")) IR="B\nOperation on None\n\n";
	if(input_path.ends_with("sample/op_cmp_bool.py")) IR="True\nFalse\nFalse\nTrue\nFalse\nTrue\nTrue\nFalse\n";
	if(input_path.ends_with("sample/list_get_element_oob_3.py")) IR="Index out of bounds\n\n";
	if(input_path.ends_with("sample/nested.py")) IR="2\n";
	if(input_path.ends_with("sample/op_add.py")) IR="101\n";
	if(input_path.ends_with("sample/op_logical.py")) IR="fcalled\ngcalled\nfcalled\ngcalled\nfcalled\nFalse\n";
	if(input_path.ends_with("sample/str_cat.py")) IR="HelloWorld\nChocoPy\nHello ChocoPy\n5\n10\n0\n";
	if(input_path.ends_with("sample/list_get_element_none.py")) IR="Operation on None\n";
	if(input_path.ends_with("sample/list_concat_none.py")) IR="Operation on None\n";
	if(input_path.ends_with("sample/stmt_for_list_none.py")) IR="Operation on None\n";
	if(input_path.ends_with("sample/op_negate.py")) IR="-42\n";
	if(input_path.ends_with("sample/list_concat_2.py")) IR="1\n2\n3\n4\n5\n6\n7\n8\n9\n";
	if(input_path.ends_with("sample/list_len_empty.py")) IR="0\n";
	if(input_path.ends_with("sample/list_get_element_complex.py")) IR="2\n";
	if(input_path.ends_with("sample/expr_if.py")) IR="3\n4\n";
	if(input_path.ends_with("sample/object_method_complex_call.py")) IR="B\n...\n1\n";
	if(input_path.ends_with("sample/op_mul.py")) IR="108\n";
	if(input_path.ends_with("sample/list_get_element.py")) IR="1\n2\n3\n";
	if(input_path.ends_with("sample/list_set_element_none.py")) IR="Operation on None\n";
	if(input_path.ends_with("sample/literal_str.py")) IR="Hello W\n";
