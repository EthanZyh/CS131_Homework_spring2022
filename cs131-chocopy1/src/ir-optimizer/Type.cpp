...
int Type::get_size() {
    if (this->is_integer_type()) {
        auto bits = dynamic_cast<IntegerType *>(this)->get_num_bits() / 8;
...
ArrayType *ArrayType::get(Type *contained) { return contained->get_module()->get_array_type(contained); }
...
Union::Union(vector<Type *> contained, string name)
    : Value(this), Type(Type::type::UNION, contained.at(0)->get_module()), contained_(contained),
      name_(std::move(name)) {}
...
string Union::print() {
    ...
    if (this->get_module()->is_declaration_) {
        ...
        for (int i = 0; i < contained_.size(); i++) {
            type_ir += "  " + contained_.at(i)->print();
            if (i != contained_.size() - 1)
                type_ir += ",";
            type_ir += +"\n";
        }
        type_ir += "}\n";
