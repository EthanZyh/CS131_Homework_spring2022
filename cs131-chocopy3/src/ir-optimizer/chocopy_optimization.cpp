...
void DeadCodeDelete::deleteDeadFunc(Module *m) {
    ...
    while (changed) {
        ...
        for (auto itfunc : func_list) {
            if (!itfunc->get_basic_blocks().empty() && itfunc->get_use_list().empty() && !is_main(itfunc)) {
...
void DeadCodeDelete::CompleteWorklist(Instruction *inst, std::unordered_set<Instruction *> &worklist) {
...
void DeadCodeDelete::dealStore(StoreInst *store, std::unordered_set<StoreInst *> &pre_stores,
                               std::unordered_set<LoadInst *> &pre_loads, std::vector<Instruction *> &wait_remove) {
    auto l_val = store->get_lval();
...
PassManager::PassManager(Module *m) : m_(m) {
    ...
    // REGISTER_CLASS(DeadCodeDelete, std::forward<Module *&>(m_));
}
